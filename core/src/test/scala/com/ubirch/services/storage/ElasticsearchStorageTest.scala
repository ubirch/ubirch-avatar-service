...
/**
  ...
  */
class ElasticsearchStorageTest extends AsyncFeatureSpec
  with Matchers
  ...
  feature("simple CRUD tests") {
    ...
    scenario("store") {
      ...
      ElasticsearchStorage.storeDoc(docIndex, docType, testDoc.id, jval).map { rjval =>
    ...
    ignore("failed get") {
      val f = Await.result(ElasticsearchStorage.getDoc("", "", ""), 5 seconds)
      f.isDefined shouldBe true
    }

    scenario("get") {
      ElasticsearchStorage.getDoc(docIndex, docType, testDoc.id).map {
        case Some(jval) =>
    ...
    scenario("update") {
      ...
      Await.ready(ElasticsearchStorage.storeDoc(docIndex, docType, testDoc2.id, jval), 2 seconds)
      ElasticsearchStorage.getDoc(docIndex, docType, testDoc2.id).map {
        case Some(jValue) =>
    ...
    scenario("getAll") {
      Thread.sleep(500)
      ElasticsearchStorage.getDocs(docIndex, docType).map {
    ...
    scenario("delete") {
      ElasticsearchStorage.deleteDoc(docIndex, docType, testDoc.id).map { res =>
        res shouldBe true
      }
    }
  }
