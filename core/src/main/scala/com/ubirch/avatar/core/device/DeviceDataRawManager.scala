...
object DeviceDataRawManager extends MyJsonProtocol
  ...
  def payloadHash(data: DeviceDataRaw): String = {
    val pStr = Json4sUtil.jvalue2String(data.p)
    val pStrHash = HashUtil.sha512HexString(pStr)

    val cStr = data.ch.getOrElse("no-ancestor")

    HashUtil.sha256HexString(s"$pStr::$cStr")
  }
}
