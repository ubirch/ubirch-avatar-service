...
object DeviceManager {
  ...
  def all(): Future[Seq[Device]] = {
    DeviceDataStorage.getDocs("devices", "device").map { res =>
      res.map { jv =>
        jv.extract[Device]
      }
    }
  ...
  def create(device: Device): Future[Option[Device]] = {
    Json4sUtil.any2jvalue(device) match {
      case Some(devJval) =>
        DeviceDataStorage.storeDoc("devices", "device", device.deviceId, devJval).map { resJval =>
  ...
  def info(deviceId: String): Future[Option[Device]] = {
    DeviceDataStorage.getDoc("devices", "device", deviceId).map {
      case Some(resJval) =>
        Some(resJval.extract[Device])
      case None => None
    }
