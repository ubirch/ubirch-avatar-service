...
class AwsConsumerActor extends Consumer with ActorLogging {
  ...
  override def endpointUri = s"aws-sqs://${Config.awsSqsQueueTransformer}?accessKey=$accessKey&secretKey=$secretKey&concurrentConsumers=2&maxMessagesPerPoll=5"
      ...
      msg.body match {
        case idStr: String =>
          try {
            ...
            DeviceDataRawManager.loadById(id = id).map {
              case Some(drd) =>
