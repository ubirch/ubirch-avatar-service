...
function build_software() {
	...
	rsync -r ~/.coursier/ ./.coursier/
  	docker run --ulimit nofile=10000:10000 --user `id -u`:`id -g` --env CLOUDREPO_USER=$CLOUDREPO_USER --env CLOUDREPO_PW=$CLOUDREPO_PW --volume=${PWD}:/build ubirch/sbt-build:${SBT_CONTAINER_VERSION} $1
